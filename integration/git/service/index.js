...
router.post('/cmd', async (ctx) => {
  ...
  // Only allow a single sub directory
  const wd = ctx.request.body.workingDir;
  const [validFilename] = wd.match(/^[^<>:;,?"*|/\\]+$/);
  if (!validFilename) {
    ctx.status = 400;
    return;
  }
  const cwd = join(process.cwd(), wd);
  const result = await runCommand('git', args, { shell: true, cwd });
  ctx.body = result;
});
