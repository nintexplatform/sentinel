...
module.exports = function () {
    this.Then(/^the application is spidered/, async function () {
        ...
        let done = await zap.checkSpiderStatus(scanID);
        while (!done) {
            await sleep(500);
            done = await zap.checkSpiderStatus(scanID);
        }
    });
    ...
    this.Then(/^the active scanner is run/, async function () {
        ...
        let done = await zap.checkActiveScanStatus(scanID);
        while (!done) {
            await sleep(500);
            done = await zap.checkActiveScanStatus(scanID);
        }
    });
